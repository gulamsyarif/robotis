...
void WalkingModule::initialize(const int control_cycle_msec, robotis_framework::Robot *robot)
{
  ...
  walking_param_.balance_hip_pitch_gain = 0.5;
...
void WalkingModule::queueThread()
{
  ...
  // Custom code
  orientation_pid_pub_ = ros_node.advertise<open_cr_module::OrientationRPY>("/orientation/pid", 1);
                            ...
                            std::map<std::string, double> sensors)
{
  ...
  else if (walking_state_ == WalkingReady || walking_state_ == WalkingEnable)
  {
    ...
    /* Original code: get gyro value from opencr */
    ...
    /* Custom code: get orientation value from opencr */
    double roll_orientation_err = (0.0 - (sensors["roll"]) * DEGREE2RADIAN);
    double pitch_orientation_err = (0.0 - (sensors["pitch"]) * DEGREE2RADIAN);

    roll_orientation_err *= fabs(roll_orientation_err) > (40 * DEGREE2RADIAN) ? 0 : 1;
    pitch_orientation_err *= fabs(pitch_orientation_err) > (60 * DEGREE2RADIAN) ? 0 : 1;

    if ((walking_param_.balance_gyro && walking_param_.balance_pid) || is_pid_balance) {
      sensoryFeedback(rl_gyro_err, fb_gyro_err, balance_angle);  
      coordOffsetFeedback(pitch_orientation_err);
    } else if (walking_param_.balance_pid) {
      coordOffsetFeedback(pitch_orientation_err);
    } else {
      sensoryFeedback(rl_gyro_err, fb_gyro_err, balance_angle);  
    }
    ...
    {
      /*
        In the Eigen3 library, the coeff() function is used to access the coefficient (i.e. the value) of a specific element in a matrix. For example, goal_position_.coeff(0, idx) accesses the element at row 0 and column "idx" of the matrix goal_position_. 
        The coeffRef() function is similar to the coeff() function, but it is used to access the element in a matrix for the purpose of modifying its value. For example, target_position_.coeffRef(0, idx) = goal_position assigns the value of the variable goal_position to the element at row 0 and column "idx" of the matrix target_position_. (by chatgpt)
      */
      ...
      // Asumsi awal error ini hanya melihat posisi robot sudah sesuai apa belum terhadap target
      double err = fabs(target_position_.coeff(0, idx) - goal_position_.coeff(0, idx)) * RADIAN2DEGREE;
      if (err > err_max)
        err_max = err;
      err_total += err;
      // ROS_INFO("ERROR TOTAl 1: %.5f", err_total);
    }

    // Check Enable
    // Walking_state tidak pernah WalkingEnable?
    if (walking_state_ == WalkingEnable && err_total > 5.0)
    {
      ROS_INFO("ERROR TOTAl 2: %.5f", err_total);
      if (DEBUG)
      ...
      ROS_WARN_STREAM_COND(DEBUG, "balance_hip_pitch_gain: " << walking_param_.balance_hip_pitch_gain);
...
void WalkingModule::sensoryFeedback(const double &rlGyroErr, const double &fbGyroErr, double *balance_angle)
{
  ...
  //     * rlGyroErr * walking_param_.balance_hip_pitch_gain;  // R_HIP_PITCH
  // balance_angle[joint_table_["l_hip_pitch"]] =  op3_kd_->getJointDirection("l_hip_pitch") * internal_gain
  //     * rlGyroErr * walking_param_.balance_hip_pitch_gain;  // L_HIP_PITCH
...
double WalkingModule::lowPassFilter(double alpha, double x_input, double& x_old)
{
  double filtered_value = x_input * alpha + x_old * (1 - alpha);
  x_old = x_input;
  return filtered_value;
}

// Custom function
void WalkingModule::sensoryFeedbackOrientation(const double &rollOrientationErr, const double &pitchOrientationErr, double *balance_angle)
{
  // adjust balance offset
  if (walking_param_.balance_enable == false)
    return;

  double internal_gain = 1;
  double p_gain_ = walking_param_.balance_p_gain;
  double i_gain_ = walking_param_.balance_i_gain;
  double d_gain_ = walking_param_.balance_d_gain;

  ros::Time curr_time = ros::Time::now();
  ros::Duration dur = curr_time - prev_time_;
  double delta_time = dur.nsec * 0.000000001 + dur.sec;

  // double delta_time = 1/30;

  double di_gain = 0.05;

  // PID controller roll balance
  double x_error_diff = (rollOrientationErr - current_roll) / delta_time;
  roll_error_sum += rollOrientationErr * delta_time;
  if (roll_error_sum > 30 * DEGREE2RADIAN)
    roll_error_sum = 30 * DEGREE2RADIAN;
  else if (roll_error_sum < -30 * DEGREE2RADIAN)
    roll_error_sum = -30 * DEGREE2RADIAN;
  
  double roll_error_target = rollOrientationErr * p_gain_ + x_error_diff * d_gain_ + roll_error_sum * i_gain_;

  // PID controller pitch balance
  double y_error_diff = (pitchOrientationErr - current_pitch) / delta_time;
  pitch_error_sum += pitchOrientationErr;
  // ROS_INFO("Error Sum Raw | Pitch: %.3f", pitch_error_sum);
  if (pitch_error_sum > 30 * DEGREE2RADIAN)
    pitch_error_sum = 30 * DEGREE2RADIAN;
  else if (pitch_error_sum < -30 * DEGREE2RADIAN)
    pitch_error_sum = -30 * DEGREE2RADIAN;

  double pitch_error_target = pitchOrientationErr * p_gain_ + y_error_diff * d_gain_ + pitch_error_sum * i_gain_;

  roll_error_target = lowPassFilter(0.001, roll_error_target, old_roll);
  pitch_error_target = lowPassFilter(0.001, pitch_error_target, old_pitch);
  ROS_INFO("Error Sum | Roll: %.3f | Pitch: %.3f", roll_error_sum, pitch_error_sum);

  balance_angle[joint_table_["r_hip_roll"]] = op3_kd_->getJointDirection("r_hip_roll") * internal_gain
      * roll_error_target * walking_param_.balance_hip_roll_gain;  // R_HIP_ROLL
  balance_angle[joint_table_["l_hip_roll"]] = op3_kd_->getJointDirection("l_hip_roll") * internal_gain
      * roll_error_target * walking_param_.balance_hip_roll_gain;  // L_HIP_ROLL

  balance_angle[joint_table_["r_hip_pitch"]] = op3_kd_->getJointDirection("r_hip_pitch") * internal_gain
      * pitch_error_target * walking_param_.balance_hip_pitch_gain;  // R_HIP_PITCH
  balance_angle[joint_table_["l_hip_pitch"]] = op3_kd_->getJointDirection("l_hip_pitch") * internal_gain
      * pitch_error_target * walking_param_.balance_hip_pitch_gain;  // L_HIP_PITCH

  balance_angle[joint_table_["r_knee"]] = op3_kd_->getJointDirection("r_knee") * internal_gain
      * pitch_error_target * walking_param_.balance_knee_gain;  // R_KNEE
  balance_angle[joint_table_["l_knee"]] = op3_kd_->getJointDirection("l_knee") * internal_gain
      * pitch_error_target * walking_param_.balance_knee_gain;  // L_KNEE

  balance_angle[joint_table_["r_ank_pitch"]] = op3_kd_->getJointDirection("r_ank_pitch")
      * internal_gain * pitch_error_target * walking_param_.balance_ankle_pitch_gain;  // R_ANKLE_PITCH
  balance_angle[joint_table_["l_ank_pitch"]] = op3_kd_->getJointDirection("l_ank_pitch")
      * internal_gain * pitch_error_target * walking_param_.balance_ankle_pitch_gain;  // L_ANKLE_PITCH

  balance_angle[joint_table_["r_ank_roll"]] = op3_kd_->getJointDirection("r_ank_roll") * internal_gain
      * roll_error_target * walking_param_.balance_ankle_roll_gain;  // R_ANKLE_ROLL
  balance_angle[joint_table_["l_ank_roll"]] = op3_kd_->getJointDirection("l_ank_roll") * internal_gain
      * roll_error_target * walking_param_.balance_ankle_roll_gain;  // L_ANKLE_ROLL

  ROS_INFO("Ankle Angle: %.3f", (op3_kd_->getJointDirection("l_ank_pitch") * RADIAN2DEGREE));
  ROS_INFO("Balance Ankle Angle: %.3f", (balance_angle[joint_table_["l_ank_pitch"]] * RADIAN2DEGREE));
  ROS_INFO("Pitch Error Target: %.3f", (pitch_error_target * RADIAN2DEGREE));

  current_roll = rollOrientationErr;
  current_pitch = pitchOrientationErr;

  prev_time_ = curr_time;

  orientation_msg_.header.stamp = ros::Time::now();
  orientation_msg_.header.frame_id = "Orientation/PID";
  orientation_msg_.pitch = pitch_error_target * RADIAN2DEGREE;
  orientation_msg_.roll = roll_error_target * RADIAN2DEGREE;
  orientation_msg_.yaw = -1;
  orientation_pid_pub_.publish(orientation_msg_);
}

void WalkingModule::sensoryFeedbackCombined(const double &rollOrientationErr, const double &pitchOrientationErr, double *balance_angle)
{
  // adjust balance offset
  if (walking_param_.balance_enable == false)
    return;
  ...
  ROS_INFO("Ankle Angle: %.3f", balance_angle[joint_table_["l_ank_pitch"]] * RADIAN2DEGREE);
...
// Custom function
void WalkingModule::coordOffsetFeedback(const double &pitchOrientationErr)
{
  // adjust balance offset
  if (walking_param_.balance_enable == false)
    return;
  ...
  // PID controller pitch balance
  double y_error_diff = (pitchOrientationErr - current_pitch) / delta_time;
  pitch_error_sum += pitchOrientationErr * delta_time ;

  // Integral anti windup
  if (pitch_error_sum > 1 * DEGREE2RADIAN)
    pitch_error_sum = 1 * DEGREE2RADIAN;
  else if (pitch_error_sum < -1 * DEGREE2RADIAN)
    pitch_error_sum = -1 * DEGREE2RADIAN;

  double pitch_error_target = pitchOrientationErr * p_gain_ + y_error_diff * d_gain_ + pitch_error_sum * i_gain_;
  current_pitch = pitchOrientationErr;
  
  double x_offset_temp = 0.3 * tan(pitch_error_target) - 0.028; 
  double z_offset_temp = 0.2 * fabs(sin(pitch_error_target)) + 0.035;

  walking_param_.init_x_offset = x_offset_temp;
  walking_param_.init_z_offset = z_offset_temp;
}
...
void WalkingModule::loadWalkingParam(const std::string &path)
{
  ...
  walking_param_.balance_hip_pitch_gain = doc["balance_hip_pitch_gain"].as<double>();
...
void WalkingModule::saveWalkingParam(std::string &path)
{
  ...
  out_emitter << YAML::Key << "balance_hip_pitch_gain" << YAML::Value << walking_param_.balance_hip_pitch_gain;
