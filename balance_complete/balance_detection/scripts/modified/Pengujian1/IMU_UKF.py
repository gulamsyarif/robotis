...
# Di bagian noise covariance pada fungsi imu_callback:
process_noise = np.eye(3) * 0.001  # Lebih kecil untuk smoothing lebih agresif
measurement_noise = np.eye(3) * 0.5  # Lebih besar agar lebih mengandalkan hasil filter

# Pada fungsi predict_sigma_points, bisa disesuaikan seperti berikut:
def predict_sigma_points(sigma_points, dt):
    ...
    for i, sp in enumerate(sigma_points):
        # Gunakan model prediksi yang lebih realistis jika memungkinkan
        predicted_sigma_points[i] = sp  # Sesuaikan berdasarkan model dinamis sistem robot Anda
