...
for i, (df, label) in enumerate(zip(dataframes, plot_labels)):
    # Plot CoM x
    ax[0].plot(df['zmp_x'], label=label)
    ax[0].set_title('ZMP X', fontsize=12)
    ax[0].set_xlabel('Index', fontsize=10)
    ax[0].set_ylabel('zmp_x', fontsize=10)

    # Plot CoM y
    ax[1].plot(df['zmp_y'], label=label)
    ax[1].set_title('ZMP Y', fontsize=12)
    ax[1].set_xlabel('Index', fontsize=10)
    ax[1].set_ylabel('zmp_y', fontsize=10)
