...
# Inisialisasi variabel global untuk menyimpan data keseimbangan
balance_text = None

# Fungsi untuk memeriksa kondisi keseimbangan
...
def update_frame(i):
    ...
    # Plot X-axis data
    axs[0].fill_between(range(start_idx, i), x_min, x_max, color='lightblue', label='Support Polygon Area')
    axs[0].plot(range(start_idx, i), com_x_data[start_idx:i], color='green', linestyle='--', linewidth=2, label='CoM')
    axs[0].plot(range(start_idx, i), zmp_x_data[start_idx:i], color='red', linewidth=2, label='ZMP')
    axs[0].set_title("Sumbu X", fontsize=16)  # Hapus parameter pad
    axs[0].set_xlabel('Data [index]')
    axs[0].set_ylabel('Nilai (cm)')

    # Plot Y-axis data
    axs[1].fill_between(range(start_idx, i), y_min, y_max, color='lightblue', label='Support Polygon Area')
    axs[1].plot(range(start_idx, i), com_y_data[start_idx:i], color='green', linestyle='--', linewidth=2, label='CoM')
    axs[1].plot(range(start_idx, i), zmp_y_data[start_idx:i], color='red', linewidth=2, label='ZMP')
    axs[1].set_title("Sumbu Y", fontsize=16)  # Hapus parameter pad
    axs[1].set_xlabel('Data [index]')
    axs[1].set_ylabel('Nilai (cm)')

    # Check balance condition
    balance_condition = check_balance(zmp_x_data[i-1], zmp_y_data[i-1], com_x_data[i-1], com_y_data[i-1], x_min, x_max, y_min, y_max)
...
def real_time_plot():
    global axs, fig, com_x_data, com_y_data, zmp_x_data, zmp_y_data, x_min, x_max, y_min, y_max
    ...
    for i in range(1, 1000):  # Ganti dengan jumlah iterasi yang sesuai
        ...
        if data:
            ...
            # Panggil fungsi untuk memperbarui frame
            update_frame(i)

        plt.pause(0.1)  # Allow for the plot to refresh

    plt.show()  # Show the plot window when done
...
def main():
    ...
    real_time_plot()  # Start the real-time plotting
