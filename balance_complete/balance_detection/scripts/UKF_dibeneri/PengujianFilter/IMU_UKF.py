...
# Rentang parameter
alpha_values = [0.001, 0.1, 0.5]
beta_values = [1, 2, 3]
kappa_values = [0]
process_noise_values = [0.001, 0.1, 0.5]
measurement_noise_values = [0.001, 0.1, 0.5]

# Output directory untuk menyimpan hasil
output_dir = r'C:\Users\syari\OneDrive\Documents\Skripsii\balance_complete\balance_detection\scripts\UKF_dibeneri\PengujianFilter\UKF'
os.makedirs(output_dir, exist_ok=True)

# Loop untuk semua kombinasi parameter
for alpha, beta, kappa, q_value, r_value in product(alpha_values, beta_values, kappa_values, process_noise_values, measurement_noise_values):
    # Hitung parameter lambda dan gamma
    n = 3  # Dimensi state (gyro x, y, z)
    lambda_ = alpha**2 * (n + kappa) - n
    gamma = np.sqrt(n + lambda_)

    # Tetapkan noise covariance
    process_noise = np.eye(n) * q_value
    measurement_noise = np.eye(n) * r_value

    # Nama file untuk hasil kombinasi parameter
    file_name = f"UKF_alpha{alpha}_beta{beta}_kappa{kappa}_Q{q_value}_R{r_value}.csv"
    file_path = os.path.join(output_dir, file_name)

    # Filter data dengan kombinasi parameter ini
    try:
        denoised_gyro = ukf_denoise(gyro_data, process_noise, measurement_noise, dt)

        # Simpan hasil filter ke file CSV
        denoised_df = pd.DataFrame(denoised_gyro, columns=['Denoised_Gyro_x', 'Denoised_Gyro_y', 'Denoised_Gyro_z'])
        denoised_df.to_csv(file_path, index=False)
        print(f"Hasil disimpan ke {file_path}")
    except Exception as e:
        print(f"Error dengan parameter alpha={alpha}, beta={beta}, kappa={kappa}, Q={q_value}, R={r_value}: {e}")
        continue
