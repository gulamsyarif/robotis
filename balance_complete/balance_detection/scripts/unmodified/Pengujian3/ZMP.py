...
def calculate_dynamic_zmp(CoM_x, CoM_y, CoM_z):
    ...
    # Mengasumsikan percepatan vertikal (ddot_CoM_z) diambil dari akselerasi IMU pada sumbu z
    ddot_CoM_z = accel[2] - GRAVITY  # percepatan vertikal berdasarkan IMU yang sudah dikurangi gravitasi

    # Hitung ZMP menggunakan formula
...
# def calculate_zmp(CoM, total_mass):
#     g = 9.81  # percepatan gravitasi
#     ZMP_x = (CoM[0] * total_mass) / (total_mass * g)
#     ZMP_y = (CoM[1] * total_mass) / (total_mass * g)
#     return ZMP_x, ZMP_y
...
#         if CoM is not None:
#             total_mass = sum(servo["mass"] for servo in servo_data)  # Hitung total massa
#             ZMP = calculate_zmp(CoM, total_mass)
#             rospy.loginfo("Calculated ZMP: x={}, y={}".format(ZMP[0], ZMP[1]))
#         rate.sleep()

# Parameter tambahan untuk perhitungan gaya
COEFFICIENT_FRICTION = 0.5  # Koefisien gesek antara kaki robot dan permukaan
FOOT_BASE_WIDTH = 0.1  # Lebar basis kaki (m)
FOOT_BASE_LENGTH = 0.2  # Panjang basis kaki (m)
...
def detect_support_phase(joint_states, imu_accel_z):
    ...
    for l_ank, r_ank, accel in zip(l_ank_pitch, r_ank_pitch, imu_accel_z):
        if l_ank > 0 and r_ank > 0:  # Kedua sendi positif
            ...
            if accel < -0.35:  # Threshold untuk mendeteksi ketika kaki mengangkat
                current_phase = 'Single Support'
            else:
                current_phase = 'Double Support'
