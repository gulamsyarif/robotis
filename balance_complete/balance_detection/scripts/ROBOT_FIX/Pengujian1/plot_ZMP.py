...
for (file_path, label), offset in zip(csv_files_with_labels, offsets):
    ...
    df_filtered = df.loc[data_range[0]:data_range[1], ['zmp_x', 'zmp_y']].reset_index(drop=True)
...
# Pengaturan plot untuk zmp_x dan zmp_y tanpa sharex
fig, axes = plt.subplots(2, 1, figsize=(12, 8))
components = ['zmp_x', 'zmp_y']
titles = ['Comparison for ZMP X', 'Comparison for ZMP Y']
y_labels = ['ZMP X (cm)', 'ZMP Y (cm)']  # Custom y-axis labels
fig.suptitle("Comparison of ZMP Coordinates Across Filtering Methods while Robot in Kicking Condition", fontsize=16)
...
for i, component in enumerate(components):
    ...
    axes[1].legend(loc='lower right', bbox_to_anchor=(1.0, 1.1), borderaxespad=0.)
