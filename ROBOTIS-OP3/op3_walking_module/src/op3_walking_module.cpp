...
void WalkingModule::queueThread()
{
  ...
  imu_data_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
    "/robotis/open_cr/imu", 10, std::bind(&WalkingModule::imuDataCallback, this, std::placeholders::_1));
...
void WalkingModule::imuDataCallback(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  ...
  // updatePitchOffset(pitch);
}

void WalkingModule::updatePitchOffset(double current_pitch)
{
    // Tentukan gain untuk adjustment (bisa disesuaikan)
    const double pitch_adjust_gain = 0.1;
    
    // Hitung error pitch (target pitch adalah 0)
    double pitch_error = 0.0 - current_pitch;
    
    
    // Logika penyesuaian tambahan
    if (current_pitch > 10) 
    {
        // Robot miring ke depan (pitch positif)
        walking_param_.init_pitch_offset += pitch_error * pitch_adjust_gain * DEGREE2RADIAN;
        walking_param_.init_pitch_offset = -fabs(walking_param_.init_pitch_offset);
    }
    else if (current_pitch < -2)
    {
        // Robot miring ke belakang (pitch negatif)
        walking_param_.init_pitch_offset += pitch_error * pitch_adjust_gain * DEGREE2RADIAN;
        walking_param_.init_pitch_offset = fabs(walking_param_.init_pitch_offset);
    }
    // Jika current_pitch == 0, pertahankan nilai offset yang ada

    // Batasi nilai offset jika diperlukan
    // const double max_pitch_offset = 10.0 * DEGREE2RADIAN;
    // walking_param_.init_pitch_offset = std::max(-max_pitch_offset, 
    //                                          std::min(max_pitch_offset, walking_param_.init_pitch_offset));
    
    // RCLCPP_INFO(this->get_logger(), 
    //            "Pitch: %.2f° -> Adjusted Offset: %.2f°", 
    //            current_pitch,
                            ...
                            std::map<std::string, double> sensors)
{
  ...
  else if (walking_state_ == WalkingReady || walking_state_ == WalkingEnable)
  {
    ...
    // RCLCPP_INFO(this->get_logger(), "Roll Gyro Error: %f", roll);
    // RCLCPP_INFO(this->get_logger(), "Pitch Gyro Error: %f", pitch);
